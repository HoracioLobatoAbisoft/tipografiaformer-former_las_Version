...
''' <summary>
'''This class manage persistency on db of Macchinario object
''' </summary>
''' <remarks>
'''
''' </remarks>

Public MustInherit Class _MacchinariDAO
	Inherits LUNA.LunaBaseClassDAO(Of Macchinario)
	...
	''' </summary>
	''' <returns>
	'''Return a Macchinario object
	''' </returns>
	Public Overrides Function Read(Id as integer) as Macchinario
		Dim cls as new Macchinario
	...
	''' </returns>
	Public Overrides Function Save(byRef cls as Macchinario) as Integer
	...
	''' </summary>
	Public Overrides Sub Delete(byref obj as Macchinario, Optional ByRef ListaObj as List (of Macchinario) = Nothing)
		DestroyPermanently (obj.IdMacchinario)
		If Not ListaObj Is Nothing Then ListaObj.Remove(obj)
 
	End Sub

	Private Function InternalFind(ByVal OrderBy As String, ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) As Macchinario
		Dim ris As Macchinario = Nothing
		Dim So As New LUNA.LunaSearchOption With {.Top = 1, .OrderBy = OrderBy}
		Dim l As IEnumerable(Of Macchinario) = FindReal(So, Parameter)
		If l.Count Then
			ris = l(0)
		End If
		Return ris
	...
	''' </summary>
	''' <returns>
	'''Return first of Macchinario
	''' </returns>
	Public Overrides Function Find(ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) As Macchinario
	...
	Public Overloads Function Find(ByVal OrderBy As String, ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) As Macchinario
		Return InternalFind(OrderBy, Parameter)
	End Function
		
	''' <summary>
	'''Find one on DB table Macchinario
	''' </summary>
	''' <returns>
	'''Return first of Macchinario
	''' </returns>
	Public Overloads Function Find(ByVal OrderBy As LUNA.LunaFieldName, ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) As Macchinario
	...
	''' </summary>
	''' <returns>
	'''Return a list of Macchinario
	''' </returns>
	Public Overrides Function FindAll(ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) as IEnumerable(Of Macchinario)
	...
	''' </returns>
	<Obsolete("Use Instead Constructor with LFM field definition")>
	Public Overloads Function FindAll(ByVal OrderBy As String, ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) as IEnumerable(Of Macchinario)
	...
	''' </returns>
	<Obsolete("Use Instead Constructor with LFM field definition")>
	Public Overloads Function FindAll(byVal Top as integer, ByVal OrderBy As String, ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) as IEnumerable(Of Macchinario)
	...
	'''Return a list of Macchinario
	''' </returns>
	Public Overloads Function FindAll(ByVal OrderBy As Luna.LunaFieldName, ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) as IEnumerable(Of Macchinario)
	...
	'''Return a list of Macchinario
	''' </returns>
	Public Overloads Function FindAll(byVal Top as integer, ByVal OrderBy As Luna.LunaFieldName, ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) as IEnumerable(Of Macchinario)
	...
	'''Return a list of Macchinario
	''' </returns>
	Public Overloads Function FindAll(ByVal SearchOption As LUNA.LunaSearchOption, ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) As IEnumerable(Of Macchinario)
	...
	''' </summary>
	''' <returns>
	'''Return a list of Macchinario by custom query
	''' </returns>
	Public Function GetBySQL(SQlQuery As String, Optional AddEmptyItem As Boolean = False) As IEnumerable(Of Macchinario)
	...
	Private Function FindReal(ByVal SearchOption As LUNA.LunaSearchOption, ByVal ParamArray Parameter() As LUNA.LunaSearchParameter) as iEnumerable(Of Macchinario)
		Dim Ls As New List(Of Macchinario)
		Try
			Dim sql As String = ""
			sql ="SELECT "   
	...
	''' </summary>
	''' <returns>
	'''Return all record in list of Macchinario
	''' </returns>
	Public Overloads Function GetAll() As IEnumerable(Of Macchinario)
	...
	'''Return all record in list of Macchinario
	''' </returns>
	Public Overloads Function GetAll(Optional OrderByField As LUNA.LunaFieldName = Nothing, Optional ByVal AddEmptyItem As Boolean = False) As IEnumerable(Of Macchinario)
	...
	''' </returns>
	<Obsolete("Use Instead Constructor with LFM field definition")>
	Public Overrides Function GetAll(Optional OrderByField As String = "",Optional ByVal AddEmptyItem As Boolean = False) As IEnumerable(Of Macchinario)
		Return InternalGetAll(OrderByField, AddEmptyItem)
	End Function

	Private Function InternalGetAll(Optional OrderByField as string = "", Optional ByVal AddEmptyItem As Boolean = False) as iEnumerable(Of Macchinario)
		Dim Ls As List(Of Macchinario)
		Try
			Dim sql As String = ""
	...
	Protected Function GetData(sql as string, Optional ByVal AddEmptyItem As Boolean = False) as iEnumerable(Of Macchinario)
		Dim Ls As New List(Of Macchinario)
		Try
			Using myCommand As DbCommand = _Cn.CreateCommand
				myCommand.CommandText = sql
				If Not LUNA.LunaContext.TransactionBox Is Nothing Then myCommand.Transaction = LUNA.LunaContext.TransactionBox.Transaction
				Using myReader As DbDataReader = myCommand.ExecuteReader()
					If AddEmptyItem Then Ls.Add(New  Macchinario() With {.IdMacchinario = 0 ,.AlertCommesse = 0 ,.AltezzaCaricoCm = 0 ,.CaricoPrevistoMensile = 0 ,.CopieOra = 0 ,.CostoMensile = 0 ,.CostoMinAvv = 0 ,.CostoSingCopia = 0 ,.Descrizione = EmptyItemDescription,.DescrizioneEstesa = "" ,.DescrizioneOnline = "" ,.HotFolderFlusso = "" ,.IdRepartoDefault = 0 ,.ImgBig = "" ,.ImgRif = "" ,.MinutiAvv = 0 ,.Ordinamento = 0 ,.Tipo = 0 ,.VisibileOnline = 0  })
					While myReader.Read
						Dim classe as new Macchinario(CType(myReader, IDataRecord))
						Ls.Add(classe)
					End While
					myReader.Close()
				End Using
