...
Public MustInherit Class _IndirizziDAO
	...
	Public Overrides Function Read(Id as integer) as Indirizzo
		...
		Try
			Using myCommand As DbCommand = _cn.CreateCommand
				myCommand.CommandText = "SELECT * FROM Indirizzi WHERE IdIndirizzo = " & Id
				...
				Using myCommand As DbCommand = _Cn.CreateCommand()
					Try
						...
						Else
							...
							If cls.WhatIsChanged.IdIndirizzoInt Then sql &= "IdIndirizzoInt = @IdIndirizzoInt,"
						...
						If cls.WhatIsChanged.IdIndirizzoInt Then 
							p = myCommand.CreateParameter
							p.ParameterName = "@IdIndirizzoInt"
							p.Value = cls.IdIndirizzoInt
							myCommand.Parameters.Add(p)
						End If
	...
	Private Sub DestroyPermanently(Id as integer) 
		Try
			Using myCommand As DbCommand = _Cn.CreateCommand
				...
				Dim Sql As String = "DELETE FROM Indirizzi"
	...
	''' <summary>
	'''Delete from DB table Indirizzi. Accept id of object to delete.
	''' </summary>
	...
	''' <summary>
	'''Delete from DB table Indirizzi. Accept object to delete and optional a List to remove the object from.
	''' </summary>
	...
	''' <summary>
	'''Find one on DB table Indirizzi
	''' </summary>
	''' <returns>
	...
	''' <summary>
	'''Find on DB table Indirizzi by custom query 
	''' </summary>
	''' <returns>
	...
	''' <summary>
	'''Return all record on DB table Indirizzi
	''' </summary>
	''' <returns>
	...
	Protected Function GetData(sql as string, Optional ByVal AddEmptyItem As Boolean = False) as iEnumerable(Of Indirizzo)
		...
		Try
			Using myCommand As DbCommand = _Cn.CreateCommand
				...
				If Not LUNA.LunaContext.TransactionBox Is Nothing Then myCommand.Transaction = LUNA.LunaContext.TransactionBox.Transaction
				Using myReader As DbDataReader = myCommand.ExecuteReader()
					If AddEmptyItem Then Ls.Add(New  Indirizzo() With {.IdIndirizzo = 0 ,.Cap = "" ,.Citta = "" ,.Destinatario = "" ,.IdComune = 0 ,.IdIndirizzoInt = 0 ,.IdNazione = 0 ,.IdProvincia = 0 ,.IdUt = 0 ,.Indirizzo = "" ,.Nome = "" ,.Predefinito = False ,.Presso = "" ,.Status = 0 ,.Telefono = ""  })
					While myReader.Read
